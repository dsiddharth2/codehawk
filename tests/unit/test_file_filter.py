...
Covers:
  ...
  - filter_changed_files: keeps code files, skips non-code by extension, skips
    deleted files regardless of extension, empty list, all-skipped scenario
"""
...
class TestFilterChangedFiles:
    ...
    def test_keeps_python_files(self):
        ...
        code, skipped = filter_changed_files(files, self._skip())
        assert len(code) == 2
    ...
    def test_empty_list(self):
        code, skipped = filter_changed_files([], self._skip())
        assert code == []
        assert skipped == []
    ...
    def test_no_skip_extensions(self):
        ...
        code, skipped = filter_changed_files(files, set())
        # With empty skip set, only deleted files are skipped
        assert len(code) == 2
        assert len(skipped) == 0
